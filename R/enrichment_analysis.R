...
#' @param data Data frame containing the outome and predictors specified in formula. Observations with a missing alue of the outcome or of any predictor are dropped. 
#' @param family Character object or call to the family() function specifying the link function that is passed to 'glm' to estimate a risk score when more than one predictor is specified. Defaults to binomial(link = "logit"), which yields logistic regression. Defaults to the logit link function to give logistic regression.
#' @param cost.screening Number indicating the cost of screening a patient to determine trial eligiblity, This argument is optional; if both cost.screening and cost.keeping are specified, then the total cost of the trial based on each screening threshold is estimated and returned.
#' @param cost.keeping Number indicating the cost of retaining a patient in the trial after enrolling. This argument is optional; if both cost.screening and cost.keeping are specified, then the total cost of the trial based on each screening threshold is estimated and returned.
#' @param do.bootstrap Logical indicating whether bootstrap 95\% confidence intervals should be computed when a dataset is specified. Defaults to TRUE.
#' @param n.bootstraps Integer indicating how many bootstrap resamples to use if do.bootstrap is TRUE. Defaults to 1000.
...
#' @param smooth.roc Logical indicating the ``smooth'' argument passed to the roc() function from the `pROC' package when the ``data'' argument is specified and a single biomarker is given.
...
#' @param reduction.under.treatment Percent reduction in event rate under treatment that the trial should be able to detect with the specified power, between 0 and 1 (exclusive). Defailts to 0.3.
...
enrichment_analysis <- function(formula,
    ...
    if (do.bootstrap == TRUE) {
        ...
        NNS.boot <- matrix(NA, nrow=n.quantiles, ncol=n.bootstraps)
        event.rate.boot <- matrix(NA, nrow=n.quantiles, ncol=n.bootstraps)
        SS.boot <- matrix(NA, nrow=n.quantiles, ncol=n.bootstraps)
        N.screen.boot <- matrix(NA, nrow=n.quantiles, ncol=n.bootstraps)
        N.screen.increase.percentage.boot <- matrix(NA, nrow=n.quantiles, ncol=n.bootstraps)
        total.cost.boot <- matrix(NA, nrow=n.quantiles, ncol=n.bootstraps)
        cost.reduction.percentage.boot <- matrix(NA, nrow=n.quantiles, ncol=n.bootstraps)
        total.cost.no.screening.boot <- rep(NA, n.bootstraps)
        ind.min.total.cost.boot <- rep(NA, n.bootstraps)
